...
vi.mock('@/lib/seat-generator', () => ({ generateSeatsForEvent: vi.fn() }))
...
describe('POST /api/admin/events/[id]/publish', () => {
  ...
  test('runs transaction with event update and seat generation on success', async () => {
    mockRequireAdmin.mockResolvedValue(ADMIN as any)
    mockFindUnique.mockResolvedValue(DRAFT_EVENT as any)
    mockFindFirst.mockResolvedValue(null)
    const fakeUpdate = vi.fn()
    mockTransaction.mockImplementation(async (fn: any) => fn({ event: { update: fakeUpdate } }))
    const res = await POST(makeRequest(), { params: { id: 'event-1' } })
    expect(res.status).toBe(200)
    expect(mockTransaction).toHaveBeenCalledOnce()
    expect(generateSeatsForEvent).toHaveBeenCalledWith(expect.anything(), 'event-1', 10)
  })
})
